@php use Illuminate\Support\Str; @endphp@extends('panel.layout.layout')
@section('title')
    ایجاد محموله
@endsection
@section('breadcrumb')
                    ...
                    <form action="{{ route('admin.shipments.store') }}" method="post" role="form">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                                    required>
                                <option value="pending" {{ old('status', 'pending') == 'pending' ? 'selected' : '' }}>در
                                                                                                                      انتظار
                                </option>
                                <option value="accepted" {{ old('status') == 'accepted' ? 'selected' : '' }}>پذیرفته
                                                                                                             شده
                                </option>
                                <option value="picked_up" {{ old('status') == 'picked_up' ? 'selected' : '' }}>تحویل
                                                                                                               گرفته شده
                                </option>
                                <option value="in_transit" {{ old('status') == 'in_transit' ? 'selected' : '' }}>در حال
                                                                                                                 انتقال
                                </option>
                                <option value="delivered" {{ old('status') == 'delivered' ? 'selected' : '' }}>تحویل
                                                                                                               داده شده
                                </option>
                            </select>
                            @error('status')
